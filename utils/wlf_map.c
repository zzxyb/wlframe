...
static struct wlf_map_node *create_node(void *key, void *value) {
	...
	if (!node) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_map_node");
		return NULL;
	}
...
/**
 * @brief Find the node for a given key.
 */
static struct wlf_map_node *find_node(struct wlf_map *map, const void *key) {
	if (!map || !key) {
		return NULL;
	}
...
/**
 * @brief Get a node color, treating NULL as black.
 */
static enum wlf_map_color node_color(struct wlf_map_node *node) {
	return node ? node->color : WLF_MAP_BLACK;
}
...
static void delete_fixup(struct wlf_map *map, struct wlf_map_node *n,
	...
	while (n != map->root && node_color(n) == WLF_MAP_BLACK) {
		if (!parent) {
			break;
		}
		...
		if (n == parent->left) {
			...
			if (node_color(w ? w->left : NULL) == WLF_MAP_BLACK &&
			    node_color(w ? w->right : NULL) == WLF_MAP_BLACK) {
				if (w) {
					w->color = WLF_MAP_RED;
				}
				n = parent;
				...
				if (node_color(w ? w->right : NULL) == WLF_MAP_BLACK) {
					if (w->left) {
...
bool wlf_map_remove(struct wlf_map *map, void *key,
	...
	struct wlf_map_node *node = find_node(map, key);
	if (!node) {
		return false; // Key not found
	}

	struct wlf_map_node *to_delete = node;
	struct wlf_map_node *child = NULL;
	struct wlf_map_node *fixup_parent = NULL;
	...
	if (!node->left) {
		...
		if (to_delete->parent == node) {
			fixup_parent = to_delete;
			if (child) {
				child->parent = to_delete;
			}
		} else {
			fixup_parent = to_delete->parent;
