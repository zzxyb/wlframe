...
void wlf_signal_init(struct wlf_signal *signal) {
	wlf_linked_list_init(&signal->listener_list);
}

void wlf_signal_add(struct wlf_signal *signal, struct wlf_listener *listener) {
	wlf_linked_list_insert(signal->listener_list.prev, &listener->link);
}
...
struct wlf_listener *wlf_signal_get(
	...
	wlf_linked_list_for_each(l, &signal->listener_list, link)
		if (l->notify == notify)
			return l;
...
void wlf_signal_emit(struct wlf_signal *signal, void *data) {
	...
	wlf_linked_list_for_each_safe(l, next, &signal->listener_list, link)
		l->notify(l, data);
...
void wlf_signal_emit_mutable(struct wlf_signal *signal, void *data) {
	...
	while (cursor.link.next != &end.link) {
		...
		wlf_linked_list_remove(&cursor.link);
		wlf_linked_list_insert(pos, &cursor.link);

		l->notify(l, data);
	}

	wlf_linked_list_remove(&cursor.link);
	wlf_linked_list_remove(&end.link);
}
