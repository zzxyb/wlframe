/**
 * @file        wlf_curve_back.h
 * @brief       Back easing curves with configurable overshoot in wlframe.
 * @details     This file provides back easing curve implementations with overshoot effects.
 ...
 *
 * This structure provides back easing curves that create an overshoot effect.
 * The base member must be the first field to enable safe casting from wlf_curve.
 */
struct wlf_curve_back {
	struct wlf_curve base;         /**< Base curve structure */
	enum wlf_curve_type type;      /**< Type of back curve */
 ...
 *
 * Safely converts a wlf_curve pointer to a wlf_curve_back pointer.
 ...
 *
 * @code
 * struct wlf_curve *curve = wlf_curve_out_back_create(1.70158f);
