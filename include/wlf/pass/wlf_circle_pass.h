 ...
 * @brief       Circle-shape rendering pass interface.
 * @details     Defines the renderer-independent options and lifecycle for
 *              submitting circle geometry.
 * @author      YaoBing Xiao
 * @date        2026-08-05
 * @version     v1.0
 * @par Copyright(c):
...
/**
 * @brief Rendering options for one circle shape.
 *
 * The shape is submitted with the specified translation, clipping, opacity,
 * and blend mode.
 */
struct wlf_render_circle_options {
	const struct wlf_circle_shape *shape; /**< Shape geometry to render. */
...
/**
 * @brief Creates a circle pass backed by a vector pass.
 *
 * The vector pass remains owned by the caller and must outlive the circle
 * pass.
 *
 * @param vector_pass Vector pass used to submit generated geometry.
 * @return Newly allocated circle pass, or NULL on failure.
 */
struct wlf_circle_pass *wlf_circle_pass_create(
	struct wlf_vector_pass *vector_pass);

/**
 * @brief Destroys a circle pass and releases its renderer resources.
 * @param pass Circle pass to destroy.
 */
void wlf_render_circle_pass_destroy(struct wlf_circle_pass *pass);

/**
 * @brief Adds one circle draw operation to the render target.
 * @param pass Circle pass receiving the draw operation.
