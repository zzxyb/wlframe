...
struct wlf_curve_bounce *wlf_curve_in_bounce_create(void) {
...
struct wlf_curve_bounce *wlf_curve_out_bounce_create(void) {
	struct wlf_curve_bounce *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_bounce");
		return NULL;
...
struct wlf_curve_bounce *wlf_curve_in_out_bounce_create(void) {
	struct wlf_curve_bounce *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_bounce");
		return NULL;
...
struct wlf_curve_bounce *wlf_curve_out_in_bounce_create(void) {
	struct wlf_curve_bounce *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_bounce");
		return NULL;
