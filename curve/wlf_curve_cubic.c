...
struct wlf_curve_cubic *wlf_curve_in_cubic_create(void) {
...
struct wlf_curve_cubic *wlf_curve_out_cubic_create(void) {
	struct wlf_curve_cubic *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_cubic");
		return NULL;
...
struct wlf_curve_cubic *wlf_curve_in_out_cubic_create(void) {
	struct wlf_curve_cubic *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_cubic");
		return NULL;
...
struct wlf_curve_cubic *wlf_curve_out_in_cubic_create(void) {
	struct wlf_curve_cubic *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_cubic");
		return NULL;
