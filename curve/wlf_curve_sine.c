...
struct wlf_curve_sine *wlf_curve_in_sine_create(void) {
...
struct wlf_curve_sine *wlf_curve_in_out_sine_create(void) {
	struct wlf_curve_sine *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_sine");
		return NULL;
...
struct wlf_curve_sine *wlf_curve_out_in_sine_create(void) {
	struct wlf_curve_sine *curve = malloc(sizeof(*curve));
	if (curve == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_curve_sine");
		return NULL;
