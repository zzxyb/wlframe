...
static bool image_load(struct wlf_image *image, const char *filename, bool enable_16_bit) {
	...
	if (features.has_animation) {
		...
		if (anim->frames == NULL) {
			wlf_log(WLF_ERROR, "Failed to allocate wlf_webp_frame");
			goto anim_out;
		}

		frame_stride = anim->canvas_w * 4;
		anim->_pixbuf = malloc((size_t)capacity * anim->canvas_h * frame_stride);
		if (anim->_pixbuf == NULL) {
			wlf_log(WLF_ERROR, "Failed to allocate pixbuf");
			goto anim_out;
		}
		...
		while (WebPAnimDecoderHasMoreFrames(dec)) {
			...
			if (idx >= capacity) {
				...
				if (tmp_frames == NULL || tmp_buf == NULL) {
					wlf_log(WLF_ERROR, "Failed to allocate wlf_webp_animation_info");
					goto anim_out;
				}
...
anim_out:
	...
	if (image->data == NULL) {
		wlf_log(WLF_ERROR, "Failed to allocate pixel data");
		free(file_data);
		return false;
	}
...
bool wlf_file_is_webp(const char *file_name) {
	size_t data_size;
	uint8_t *data = read_file(file_name, &data_size);
	if (data == NULL) {
		return false;
	}
	int width, height;
	int ok = WebPGetInfo(data, data_size, &width, &height);
	free(data);
	return ok != 0;
}
