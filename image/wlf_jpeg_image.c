...
static bool jpeg_image_save(struct wlf_image *image, const char *filename) {
	...
	/* Set colorspace and input components */
	switch (image->format) {
		...
		case WLF_COLOR_TYPE_RGBA:
			/* JPEG doesn't support alpha channel directly, convert to RGB */
			cinfo.input_components = 3;
			cinfo.in_color_space = JCS_RGB;
		...
		case WLF_COLOR_TYPE_GRAY_ALPHA:
			/* Convert grayscale+alpha to grayscale */
			cinfo.input_components = 1;
			cinfo.in_color_space = JCS_GRAYSCALE;
	...
	/* Set arithmetic coding if requested */
	...
	switch (jpeg_image->options.subsampling) {
		case WLF_JPEG_SUBSAMPLING_444:
			/* No subsampling - set all components to 1x1 */
	...
	/* Prepare row buffer for RGBA to RGB conversion if needed */
...
static bool jpeg_image_load(struct wlf_image *image, const char *filename, bool enable_16_bit) {
	...
	/* Note: 16-bit is not supported by standard JPEG */
	if (enable_16_bit) {
		wlf_log(WLF_INFO, "16-bit mode not supported by JPEG, using 8-bit");
	}

	/* Determine output format based on JPEG colorspace */
	...
	image->bit_depth = WLF_IMAGE_BIT_DEPTH_8; /* JPEG is always 8-bit */
	image->has_alpha_channel = false; /* JPEG doesn't support alpha */
	image->is_opaque = true;

	/* Calculate stride and allocate image data */
...
enum wlf_jpeg_colorspace wlf_color_type_to_jpeg_colorspace(struct wlf_image *image) {
	assert(wlf_jpeg_image_from_image(image) != NULL);
	if (image == NULL) {
