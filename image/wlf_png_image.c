...
#define MAX_SCREEN_COLORS 256  // Maximum number of colors to quantize to
...
static bool png_image_load(struct wlf_image *image, const char *filename, bool enable_16_bit) {
	...
	if (color_type == PNG_COLOR_TYPE_PALETTE) {
		...
		image->format = PNG_COLOR_TYPE_RGB;
		image->is_opaque = true;
	...
	wlf_log(WLF_DEBUG, "PNG Info: %dx%d, Format: %d, Bit Depth: %d\n",
		image->width, image->height, image->format, image->bit_depth);
	return true;
};
...
void wlf_png_image_print_data_gimp_style(const struct wlf_image *image) {
	...
	size_t row_stride = image->width * channels;
	...
	for (uint32_t y = 0; y < image->height; ++y) {
		const unsigned char *row = image->data + y * row_stride;
		char line[1024];
		int offset = 0;
