...
int main(int argc, char *argv[]) {
	...
	// Division by zero handling (should be handled in implementation)
	struct wlf_fpoint div_test = (struct wlf_fpoint){.x = 5.0, .y = 5.0};
	struct wlf_fpoint div_by_zero = wlf_fpoint_divide(&div_test, 0.0);
	char *div_by_zero_str = wlf_fpoint_to_str(&div_by_zero);
