/**
 * @file        wlf_ray_test.c
 * @brief       Comprehensive test suite for wlf_ray functionality.
 * @details     This file provides complete testing coverage for all wlf_ray
...
static void test_ray_creation(void) {
	...
	struct wlf_vector3 origin = wlf_vector3_create(1.0, 2.0, 3.0);
	struct wlf_vector3 direction = wlf_vector3_create(0.0, 1.0, 0.0);
	struct wlf_ray ray = wlf_ray_create(origin, direction);
	...
	struct wlf_ray zero_ray = wlf_ray_create(zero, zero);
...
void test_ray_unit_axes(void) {
	...
	// X-axis ray
	struct wlf_ray x_ray = wlf_ray_create(origin, WLF_VECTOR3_UNIT_X);
	ASSERT_VECTOR3_EQ(x_ray.direction, WLF_VECTOR3_UNIT_X, "X-axis ray direction");

	// Y-axis ray
	struct wlf_ray y_ray = wlf_ray_create(origin, WLF_VECTOR3_UNIT_Y);
	ASSERT_VECTOR3_EQ(y_ray.direction, WLF_VECTOR3_UNIT_Y, "Y-axis ray direction");

	// Z-axis ray
	struct wlf_ray z_ray = wlf_ray_create(origin, WLF_VECTOR3_UNIT_Z);
...
void test_ray_point_at_parameter(void) {
	...
	struct wlf_vector3 point_0 = wlf_ray_point_at_parameter(&ray, 0.0);
	ASSERT_VECTOR3_EQ(point_0, origin, "Point at t=0 should be origin");

	struct wlf_vector3 point_1 = wlf_ray_point_at_parameter(&ray, 1.0);
	struct wlf_vector3 expected_1 = wlf_vector3_create(1.0, 0.0, 0.0);
	ASSERT_VECTOR3_EQ(point_1, expected_1, "Point at t=1 should be (1,0,0)");

	struct wlf_vector3 point_5 = wlf_ray_point_at_parameter(&ray, 5.0);
	struct wlf_vector3 expected_5 = wlf_vector3_create(5.0, 0.0, 0.0);
	ASSERT_VECTOR3_EQ(point_5, expected_5, "Point at t=5 should be (5,0,0)");

	// Test negative parameter (behind origin)
	struct wlf_vector3 point_neg = wlf_ray_point_at_parameter(&ray, -2.0);
	struct wlf_vector3 expected_neg = wlf_vector3_create(-2.0, 0.0, 0.0);
...
void test_ray_point_at_parameter_complex(void) {
	...
	struct wlf_vector3 origin = wlf_vector3_create(1.0, 2.0, 3.0);
	struct wlf_vector3 direction = wlf_vector3_create(2.0, -1.0, 1.0);
	struct wlf_ray ray = wlf_ray_create(origin, direction);

	// Test point calculation
	struct wlf_vector3 point = wlf_ray_point_at_parameter(&ray, 2.0);
	struct wlf_vector3 expected = wlf_vector3_create(5.0, 0.0, 5.0); // (1,2,3) + 2*(2,-1,1)
	ASSERT_VECTOR3_EQ(point, expected, "Point calculation with non-unit direction");

	// Test fractional parameter
	struct wlf_vector3 point_half = wlf_ray_point_at_parameter(&ray, 0.5);
	struct wlf_vector3 expected_half = wlf_vector3_create(2.0, 1.5, 3.5);
...
void test_ray_equality(void) {
	...
	// Test identical rays
	struct wlf_vector3 origin1 = wlf_vector3_create(1.0, 2.0, 3.0);
	struct wlf_vector3 direction1 = wlf_vector3_create(0.0, 1.0, 0.0);
	struct wlf_ray ray1 = wlf_ray_create(origin1, direction1);
	struct wlf_ray ray2 = wlf_ray_create(origin1, direction1);

	ASSERT_TRUE(wlf_ray_equal(&ray1, &ray2), "Identical rays should be equal");

	// Test different origins
	struct wlf_vector3 origin2 = wlf_vector3_create(2.0, 2.0, 3.0);
	struct wlf_ray ray3 = wlf_ray_create(origin2, direction1);

	ASSERT_TRUE(!wlf_ray_equal(&ray1, &ray3), "Rays with different origins should not be equal");

	// Test different directions
	struct wlf_vector3 direction2 = wlf_vector3_create(1.0, 0.0, 0.0);
	struct wlf_ray ray4 = wlf_ray_create(origin1, direction2);

	ASSERT_TRUE(!wlf_ray_equal(&ray1, &ray4), "Rays with different directions should not be equal");

	// Test zero rays
	ASSERT_TRUE(wlf_ray_equal(&WLF_RAY_ZERO, &WLF_RAY_ZERO), "Zero ray should equal itself");
}
...
void test_ray_nearly_equal(void) {
	...
	struct wlf_vector3 origin2 = wlf_vector3_create(1.0 + 1e-10, 2.0, 3.0);
	struct wlf_vector3 direction2 = wlf_vector3_create(0.0, 1.0 + 1e-10, 0.0);
	struct wlf_ray ray2 = wlf_ray_create(origin2, direction2);

	ASSERT_TRUE(wlf_ray_nearly_equal(&ray1, &ray2, 1e-9), "Nearly identical rays should be nearly equal");
	ASSERT_TRUE(!wlf_ray_nearly_equal(&ray1, &ray2, 1e-11), "Rays outside epsilon should not be nearly equal");

	// Test exact equality with nearly_equal
	ASSERT_TRUE(wlf_ray_nearly_equal(&ray1, &ray1, 1e-15), "Ray should be nearly equal to itself");
}
...
void test_ray_string_conversion(void) {
	...
	char* ray_str = wlf_ray_to_str(&ray);
	...
	char* zero_str = wlf_ray_to_str(&WLF_RAY_ZERO);
...
void test_ray_geometric_properties(void) {
	...
	struct wlf_vector3 origin = wlf_vector3_create(2.0, 3.0, 4.0);
	struct wlf_vector3 direction = wlf_vector3_create(1.0, -1.0, 2.0);
	struct wlf_ray ray = wlf_ray_create(origin, direction);

	// Verify parameterization for multiple points
	for (int i = 0; i < 5; i++) {
		double t = i * 0.5;
		struct wlf_vector3 point = wlf_ray_point_at_parameter(&ray, t);
	...
	struct wlf_vector3 p1 = wlf_ray_point_at_parameter(&ray, t1);
	struct wlf_vector3 p_sum = wlf_ray_point_at_parameter(&ray, t1 + t2);
...
void test_ray_edge_cases(void) {
	...
	struct wlf_vector3 large_origin = wlf_vector3_create(1e6, -1e6, 1e8);
	struct wlf_vector3 small_direction = wlf_vector3_create(1e-6, 1e-8, 1e-10);
	struct wlf_ray large_ray = wlf_ray_create(large_origin, small_direction);

	struct wlf_vector3 point = wlf_ray_point_at_parameter(&large_ray, 1e6);
	...
	struct wlf_ray zero_dir_ray = wlf_ray_create(origin, zero_direction);

	struct wlf_vector3 point_zero_dir = wlf_ray_point_at_parameter(&zero_dir_ray, 10.0);
	ASSERT_VECTOR3_EQ(point_zero_dir, origin, "Ray with zero direction should always return origin");

	// Test very small epsilon
	struct wlf_ray ray1 = wlf_ray_create(origin, WLF_VECTOR3_UNIT_X);
	struct wlf_vector3 slightly_different = wlf_vector3_create(1.0 + 1e-15, 2.0, 3.0);
	struct wlf_ray ray2 = wlf_ray_create(slightly_different, WLF_VECTOR3_UNIT_X);

	ASSERT_TRUE(!wlf_ray_nearly_equal(&ray1, &ray2, 1e-16),
...
void test_ray_normalization_considerations(void) {
	...
	struct wlf_vector3 temp_vector = wlf_vector3_create(3.0, 4.0, 0.0);
	struct wlf_vector3 normalized_dir = wlf_vector3_normalize(&temp_vector);
	struct wlf_ray normalized_ray = wlf_ray_create(origin, normalized_dir);

	struct wlf_vector3 point_norm = wlf_ray_point_at_parameter(&normalized_ray, 5.0);
	...
	struct wlf_vector3 non_normalized_dir = wlf_vector3_create(3.0, 4.0, 0.0); // magnitude = 5
	struct wlf_ray non_normalized_ray = wlf_ray_create(origin, non_normalized_dir);

	struct wlf_vector3 point_non_norm = wlf_ray_point_at_parameter(&non_normalized_ray, 1.0);
...
int main(void) {
	printf("Starting wlf_ray comprehensive test suite...\n");
