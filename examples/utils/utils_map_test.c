...
static int validate_black_height(struct wlf_map_node *node) {
	if (!node) {
		return 1;
	}

	if (node->color == WLF_MAP_RED) {
		assert(!node->left || node->left->color == WLF_MAP_BLACK);
		assert(!node->right || node->right->color == WLF_MAP_BLACK);
	}
	...
	return left_height + (node->color == WLF_MAP_BLACK ? 1 : 0);
}

static void assert_map_invariants(struct wlf_map *map) {
	assert(map != NULL);
	...
	assert(map->root->color == WLF_MAP_BLACK);
	validate_black_height(map->root);
	...
	assert(iterated == wlf_map_size(map));
}
...
/* ========================================================================
 * Example 7: Regression checks
 * ======================================================================== */

static void example_regressions(void) {
	printf("\n=== Example 7: Regression Checks ===\n");

	struct wlf_map *map = wlf_map_create(int_compare);
	assert(map != NULL);

	int keys[] = {10, 5, 15, 1, 7, 12, 18, 0, 2, 6, 8};
	for (size_t i = 0; i < sizeof(keys) / sizeof(keys[0]); i++) {
		assert(wlf_map_insert(map, &keys[i], &keys[i]));
	}

	assert_map_invariants(map);

	int remove_keys[] = {0, 1, 5, 10};
	for (size_t i = 0; i < sizeof(remove_keys) / sizeof(remove_keys[0]); i++) {
		assert(wlf_map_remove(map, &remove_keys[i], NULL, NULL));
		assert_map_invariants(map);
		assert(!wlf_map_contains(map, &remove_keys[i]));
	}

	int missing_key = 999;
	assert(!wlf_map_remove(map, &missing_key, NULL, NULL));

	wlf_map_destroy(map, NULL, NULL);

	struct wlf_map *null_map = wlf_map_create(int_compare);
	assert(null_map != NULL);

	int null_key = 42;
	assert(wlf_map_insert(null_map, &null_key, NULL));
	assert(wlf_map_contains(null_map, &null_key));
	assert(wlf_map_find(null_map, &null_key) == NULL);

	wlf_map_destroy(null_map, NULL, NULL);
