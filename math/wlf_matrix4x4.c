...
#define WLF_MATRIX4X4_EPSILON 1e-12

static double determinant3x3(const double elements[3][3]) {
...
static double matrix4x4_minor(const struct wlf_matrix4x4 *matrix, int row, int col) {
	...
	for (int i = 0; i < 4; ++i) {
		...
		for (int j = 0; j < 4; ++j) {
			...
			submatrix[sub_row][sub_col++] = matrix->elements[i][j];
		}
		sub_row++;
	}
...
double wlf_matrix4x4_determinant(const struct wlf_matrix4x4 *matrix) {
	...
	for (int col = 0; col < 4; ++col) {
		double cofactor = matrix4x4_minor(matrix, 0, col);
		if (col % 2 != 0) {
			cofactor = -cofactor;
		}
		det += matrix->elements[0][col] * cofactor;
	}
...
struct wlf_matrix4x4 wlf_matrix4x4_inverse(const struct wlf_matrix4x4 *matrix) {
	...
	if (fabs(det) <= WLF_MATRIX4X4_EPSILON) {
	...
	for (int row = 0; row < 4; ++row) {
		for (int col = 0; col < 4; ++col) {
			double cofactor = matrix4x4_minor(matrix, row, col);
			if ((row + col) % 2 != 0) {
				cofactor = -cofactor;
			}

			result.elements[col][row] = cofactor / det;
		}
	}
