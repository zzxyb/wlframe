...
void wlf_clipboard_init(struct wlf_clipboard *clipboard,
		const struct wlf_clipboard_impl *impl) {
	assert(clipboard != NULL);
	assert(impl != NULL);
	assert(impl->destroy != NULL);
	assert(impl->set_data != NULL);
	assert(impl->get_data != NULL);
	assert(impl->get_mime_types != NULL);
	assert(impl->clear != NULL);
...
struct wlf_clipboard *wlf_clipboard_create(const struct wlf_clipboard_impl *impl) {
	struct wlf_clipboard *clipboard = calloc(1, sizeof(struct wlf_clipboard));
	if (clipboard == NULL) {
		wlf_log_errno(WLF_ERROR, "failed to allocate wlf_clipboard");
		return NULL;
	}

	wlf_clipboard_init(clipboard, impl);
...
void wlf_clipboard_destroy(struct wlf_clipboard *clipboard) {
	...
	assert(wlf_linked_list_empty(&clipboard->events.destroy.listener_list));
