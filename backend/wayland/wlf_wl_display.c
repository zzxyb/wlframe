...
static void display_handle_global(void *data, struct wl_registry *wl_registry,
	...
	wlf_double_list_for_each(reg, &display->interfaces, link) {
	...
	wlf_double_list_insert(&display->interfaces, &new_reg->link);
...
struct wlf_wl_display *wlf_wl_display_create(void) {
	...
	wlf_double_list_init(&display->interfaces);
...
void wlf_wl_display_destroy(struct wlf_wl_display *display) {
	...
	wlf_double_list_remove(&display->interfaces);
	free(display);
}
...
void wlf_wl_registry_destroy(struct wlf_wl_interface *registry) {
	...
	wlf_double_list_remove(&registry->link);
