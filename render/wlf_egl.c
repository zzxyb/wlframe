...
struct wlf_egl *wlf_egl_create_with_context(EGLDisplay display,
		EGLContext context) {
	EGLint client_type;
	if (!eglQueryContext(display, context, EGL_CONTEXT_CLIENT_TYPE, &client_type) ||
			client_type != EGL_OPENGL_ES_API) {
		wlf_log(WLF_ERROR, "Unsupported EGL context client type (need OpenGL ES)");
		return NULL;
	}
	...
	if (!eglQueryContext(display, context, EGL_CONTEXT_CLIENT_VERSION, &client_version) ||
			client_version < 2) {
		wlf_log(WLF_ERROR, "Unsupported EGL context client version (need OpenGL ES >= 2)");
		return NULL;
	}

	struct wlf_egl *egl = egl_create();
	if (egl == NULL) {
		return NULL;
	}
