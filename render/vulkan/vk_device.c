...
VkPhysicalDevice wlf_vk_find_phdev(struct wlf_vk_instance *ini) {
	...
	res = vkEnumeratePhysicalDevices(ini->base, &num_phdevs, NULL);
	if (res != VK_SUCCESS) {
	...
	res = vkEnumeratePhysicalDevices(ini->base, &num_phdevs, phdevs);
	if (res != VK_SUCCESS) {
		wlf_vk_error("Could not retrieve physical devices", res);
		return VK_NULL_HANDLE;
	}

	bool force_sw = wlf_env_parse_bool("WLR_RENDER_FORCE_SOFTWARE");
	bool force_discrete_gpu = wlf_env_parse_bool("WLR_RENDER_FORCE_DISCRETE_GPU");
	for (uint32_t i = 0; i < num_phdevs; ++i) {
		...
		if (force_sw) {
			wlf_log(WLF_INFO, "WLR_RENDER_FORCE_SOFTWARE env variable is set, "
				"forcing software render");
			found = phdev_props.deviceType == VK_PHYSICAL_DEVICE_TYPE_CPU;
		} else {
			if (force_discrete_gpu) {
				wlf_log(WLF_INFO, "WLR_RENDER_FORCE_DISCRETE_GPU env variable is set, "
