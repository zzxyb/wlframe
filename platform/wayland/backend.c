...
static void display_global_added(void *data, struct wl_registry *wl_registry,
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		if (version > (uint32_t)wl_subcompositor_interface.version) {
			wlf_log(WLF_DEBUG, "Server subcompositor version %u is higher than client version %u, "
					"using client version", version, (uint32_t)wl_subcompositor_interface.version);
			bind_version = (uint32_t)wl_subcompositor_interface.version;
		}

		wayland->subcompositor = wl_registry_bind(wl_registry,
			name,
			&wl_subcompositor_interface,
			bind_version);
		if (wayland->compositor == NULL) {
			wlf_log(WLF_ERROR, "Failed to bind wl_subcompositor interface with name %u", name);
			return;
		} else {
			wlf_log(WLF_DEBUG, "Successfully bound wl_subcompositor interface (name: %u, version: %u)",
					name, bind_version);
		}
	} else if (strcmp(interface, wl_seat_interface.name) == 0) {
		uint32_t bind_version = version;
		if (version > (uint32_t)wl_seat_interface.version) {
			wlf_log(WLF_DEBUG, "Server seat version %u is higher than client version %u, "
					"using client version", version, (uint32_t)wl_seat_interface.version);
			bind_version = (uint32_t)wl_seat_interface.version;
		}

		wayland->seat = wl_registry_bind(wl_registry,
			name,
			&wl_seat_interface,
			bind_version);
		if (wayland->compositor == NULL) {
			wlf_log(WLF_ERROR, "Failed to bind wl_seat interface with name %u", name);
			return;
		} else {
			wlf_log(WLF_DEBUG, "Successfully bound wl_seat interface (name: %u, version: %u)",
					name, bind_version);
		}
