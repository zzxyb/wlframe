...
static void backend_output_added(struct wlf_listener *listener, void *data) {
	struct wlf_backend_macos *macos =
		wlf_container_of(listener, macos, output_added);
	wlf_signal_emit_mutable(&macos->base.events.output_added, data);
}

static void backend_output_removed(struct wlf_listener *listener, void *data) {
	struct wlf_backend_macos *macos =
		wlf_container_of(listener, macos, output_removed);
	wlf_signal_emit_mutable(&macos->base.events.output_removed, data);
}
...
static void backend_destroy(struct wlf_backend *backend) {
	...
	if (macos->output_manager != NULL) {
		wlf_linked_list_remove(&macos->output_added.link);
		wlf_linked_list_remove(&macos->output_removed.link);
		wlf_output_manager_destroy(macos->output_manager);
		macos->output_manager = NULL;
	}
...
struct wlf_backend *macos_backend_create(void) {
	...
	backend->output_manager = wlf_macos_output_manager_create();
	if (backend->output_manager == NULL) {
		wlf_log(WLF_ERROR, "Failed to create macOS output manager");
		free(backend);
		return NULL;
	}

	backend->output_added.notify = backend_output_added;
	backend->output_removed.notify = backend_output_removed;
	wlf_signal_add(&backend->output_manager->events.output_added,
		&backend->output_added);
	wlf_signal_add(&backend->output_manager->events.output_removed,
		&backend->output_removed);

	struct wlf_output *output;
	wlf_linked_list_for_each(output, &backend->output_manager->outputs, link) {
