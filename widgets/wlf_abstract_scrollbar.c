...
void wlf_abstract_scrollbar_init(struct wlf_abstract_scrollbar *widget,
	...
	*widget = (struct wlf_abstract_scrollbar){
		.impl = impl,
		.frame = *frame,
	...
	if (widget->tree == NULL) {
		wlf_log(WLF_ERROR, "failed to create scrollbar scene tree");
		return;
	}
...
void wlf_abstract_scrollbar_set_page_size(struct wlf_abstract_scrollbar *widget,
	...
	if (widget->page_size == page_size) {
		return;
	}

	widget->page_size = page_size;
	if (widget->impl->set_page_size != NULL) {
		widget->impl->set_page_size(widget, page_size);
	}
}
...
void wlf_abstract_scrollbar_set_value(struct wlf_abstract_scrollbar *widget,
	...
	struct wlf_abstract_scrollbar_value_event event = {
		.widget = widget,
		.value = widget->value,
