...
void wlf_abstract_slider_init(struct wlf_abstract_slider *widget,
	...
	*widget = (struct wlf_abstract_slider){
		.impl = impl,
		.frame = *frame,
	...
	if (widget->tree == NULL) {
		wlf_log(WLF_ERROR, "failed to create slider scene tree");
		return;
	}
...
void wlf_abstract_slider_set_step(struct wlf_abstract_slider *widget,
	...
	assert(widget != NULL);
	widget->step = step;
	if (widget->impl->set_step != NULL) {
		widget->impl->set_step(widget, step);
	}
}
...
void wlf_abstract_slider_set_value(struct wlf_abstract_slider *widget,
	...
	if (widget->step > 0.0) {
		double steps = (value - widget->minimum) / widget->step;
		long nearest = (long)(steps + 0.5);
		value = widget->minimum + nearest * widget->step;
	}
	...
	struct wlf_abstract_slider_value_event event = {
		.widget = widget,
		.value = widget->value,
...
void wlf_abstract_slider_set_normalized_value(struct wlf_abstract_slider *widget,
		double normalized_value) {
	assert(widget != NULL);
	normalized_value = clamp01(normalized_value);
	if (widget->normalized_value == normalized_value) {
		return;
	}

	widget->normalized_value = normalized_value;
	widget->value = widget->minimum +
		(widget->maximum - widget->minimum) * normalized_value;

	if (widget->impl->set_normalized_value != NULL) {
		widget->impl->set_normalized_value(widget, normalized_value);
	}
