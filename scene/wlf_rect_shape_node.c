...
WLF_DEFINE_SHAPE_NODE(rect_shape_node, wlf_rect_shape_node,
	wlf_scene_node_is_rect_shape, wlf_rect_shape_node_from_node,
	scene_node_render)

static void bounds(const struct wlf_rect_shape *shape,
		double *minx, double *miny, double *maxx, double *maxy) {
	*minx = fmin(shape->x, shape->x + shape->width);
...
struct wlf_rect_shape_node *wlf_rect_shape_node_create(
	...
	struct wlf_rect_shape_node *node = calloc(1, sizeof(*node));
	if (node == NULL) return NULL;
	double minx, miny, maxx, maxy;
	bounds(shape, &minx, &miny, &maxx, &maxy);
	if (!wlf_shape_node_common_init(&node->base, &rect_shape_node_impl,
...
static void render_at(struct wlf_rect_shape_node *node,
	...
	wlf_render_pass_add_rect_shape(pass, target,
...
static void scene_node_render(struct wlf_render_list_entry *entry,
	...
	render_at(wlf_rect_shape_node_from_node(entry->node),
		data->scene->rect_shape_pass, data->target, &render_region,
		entry->x, entry->y);
	pixman_region32_fini(&render_region);
}
