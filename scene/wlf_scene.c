...
static struct wlf_render_target_info *begin_render(struct wlf_scene *scene) {
	...
	if (wlf_renderer_is_gles(window->state.renderer) &&
		...
		return configure_render_target(scene,
			target != NULL ? &target->base : NULL);
	}
	...
	if (wlf_renderer_is_pixman(window->state.renderer) &&
		...
		struct wlf_pixman_render_target_info *pixman_target =
			wlf_pixman_begin_pixman_render_pass(buffer);
		return configure_render_target(scene,
			pixman_target != NULL ? &pixman_target->base : NULL);
	}
